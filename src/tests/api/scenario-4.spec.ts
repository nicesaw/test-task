...
import { getItemWithoutDiscount } from 'api/helper';
...
test('Scenario 4', async ({ userApi }) => {
  ...
  await step('Add 1 items without a discount to the basket', async () => {
    const notesList = await userApi.postGetProduct();
    note = getItemWithoutDiscount(notesList);
